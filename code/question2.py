...
sys.stdout = open("test2.csv", "w",encoding="utf-8")
# 定义地块、作物、年份、季节等
plots = [
    "A1",
    "A2",
...
for year in years:
    for crop in crops:
        if crop in ["小麦", "玉米"]:
            expected_sales[crop] *= np.random.uniform(
                1.05, 1.10
            )  # 小麦和玉米年增长率5%-10%
        else:
            expected_sales[crop] *= np.random.uniform(0.95, 1.05)  # 其他作物±5%变化

        yield_per_acre[crop] = apply_uncertainty(
            yield_per_acre[crop], 0.9, 1.1
        )  # 产量±10%变化
        cost_per_acre[crop] *= 1.05  # 成本每年增长5%

        if crop in ["榆黄菇", "香菇", "白灵菇", "羊肚菌"]:
            price_per_jin[crop] *= np.random.uniform(
                0.95, 0.99
            )  # 食用菌价格每年下降1%-5%
            if crop == "羊肚菌":
...
# 创建优化模型
model = lp.LpProblem("Maximize_Profit_with_Uncertainty", lp.LpMaximize)

# 创建决策变量（整数规划）
x = lp.LpVariable.dicts(
    "Area", (plots, crops, years, seasons), lowBound=0, cat="Integer"
)
...
total_profit = lp.lpSum(
    [
        (
            price_per_jin[crop]
            * lp.lpSum([yield_per_acre[crop] * x[plot][crop][year][season]])
            + 0.5
            * price_per_jin[crop]
            * lp.lpSum(
                [
                    yield_per_acre[crop] * x[plot][crop][year][season]
                    - expected_sales[crop]
                ]
            )
...
for plot in plots:
    for year in years:
        for season in seasons:
            model += (
                lp.lpSum([x[plot][crop][year][season] for crop in crops])
                <= plot_area[plot]
            )
...
for plot in plots:
    for year in years:
        model += (
            lp.lpSum(
                [
                    x[plot][crop][year][season]
                    for crop in ["黄豆", "黑豆", "红豆", "绿豆", "豇豆", "刀豆", "芸豆"]
                    for season in seasons
                ]
            )
            >= plot_area[plot] / 3
    ...
    for year in years[:-1]:  # 遍历到倒数第二个年份
        for season in seasons:
            for crop in crops:
                ...
                model += (
                    y[plot][crop][year][season] + y[plot][crop][year + 1][season]
                ) <= 1
    ...
    for season in seasons:
        model += (
            lp.lpSum(
                [
                    [
                        y[plot][crop][year][season]
                        for plot in plots[27:35]
                        for crop in crops[16:41]
                    ]
                ]
            )
            <= 2
        ...
        model += (
            lp.lpSum(
                [
                    [
                        y[plot][crop][year][season]
                        for plot in plots[35:55]
                        for crop in crops[17:42]
                    ]
                ]
            )
            == 2
        ...
        for year in years:
            for season in seasons:
                if x[plot][crop][year][season].varValue > 0:
                    print(
                        f"{plot},{crop},{year},{x[plot][crop][year][season].varValue}"
                    )
