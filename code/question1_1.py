import pulp as lp
import sys
sys.stdout=open('output1_1.txt','w')
# 定义地块、作物、年份、季节等
...
# 情况1：超过部分滞销，造成浪费
model1 = lp.LpProblem("Maximize_Profit_Waste", lp.LpMaximize)
...
# 目标函数：最大化总利润（考虑滞销部分没有收益）

total_profit1 = lp.lpSum([
    (price_per_jin[crop] * yield_per_acre[crop] * x[plot][crop][year][season] - 
...
model1 += total_profit1

# 约束1：地块面积不得超过限制
for plot in plots:
    for year in years:
        for season in seasons:
            model1 += lp.lpSum([x[plot][crop][year][season] for crop in crops]) <= plot_area[plot]

# 约束2：每个地块三年内至少种植一次豆类作物
for plot in plots:
    for year in years:
        model1 += lp.lpSum([x[plot][crop][year][season] for crop in ['黄豆', '黑豆', '红豆', '绿豆', '豇豆', '刀豆', '芸豆'] for season in seasons]) >= plot_area[plot] / 3
# 约束2：每个地块三年内至少种植一次豆类作物
for plot in plots:
    for year in years:
        model1 += lp.lpSum([x[plot][crop][year][season] for crop in ['黄豆', '黑豆', '红豆', '绿豆', '豇豆', '刀豆', '芸豆'] for season in seasons]) >= plot_area[plot] / 3
    ...
    for year in years[:-1]:  # 遍历到倒数第二个年份
        for season in seasons:
            for crop in crops:
                ...
                model1 +=(y[plot][crop][year][season] +y[plot][crop][year+1][season]) <= 1
...
for plot in plots[:27]:
    for year in years:
        for season in seasons:
            model1+=lp.lpSum([y[plot][crop][year][season] for crop in crops[:17]]) <= 1

# 约束5:水浇地适宜每年种植一季水稻或两季蔬菜：
for year in years:
    for season in seasons:
        model1+=lp.lpSum([[y[plot][crop][year][season] for plot in plots[27:35] for crop in crops[16:41]]]) <= 2

# # 约束6:普通大棚每年种植两季作物，第一季可种植多种蔬菜（大白菜、白萝卜和红萝卜除外），第二季只能种植食用菌
# for year in years:
#     for season in seasons:
#         model1+=lp.lpSum([[y[plot][crop][year][season] for plot in plots[35:55] for crop in crops[17:42]]]) == 2


# # 约束7:智慧大棚每年都可种植两季蔬菜（大白菜、白萝卜和红萝卜除外）：
# for year in years:
#     for season in seasons:
#         model1+=lp.lpSum([[y[plot][crop][year][season] for plot in plots[51:55] for crop in crops[17:35]]]) == 1
...
#             model1+=y[plot][crop][year][1] == 0
#             model1+=y[plot][crop][year][2] == 1
#         for crop in crops[35:38]:
#             model1+=y[plot][crop][year][1] == 1
#             model1+=y[plot][crop][year][2] == 0
...
print(f"Total Profit: {lp.value(model1.objective)}")
for plot in plots:
    for crop in crops:
        for year in years:
            for season in seasons:
